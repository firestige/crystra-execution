...
async function loadCommandView(): Promise<(props: unknown) => any> {
  ...
  if (component === undefined) throw new TypeError("CRYSTRA_COMMAND_VIEW_MISSING");
  return component;
}
...
describe("DSH official Crystra command view", () => {
  it("hides every internal Crystra command lifecycle because native user/output nodes own the chat timeline", async () => {
    const view = await loadCommandView();
    expect(view({ node: {
      commandId: "command-create", name: "crystra",
    ...
    for (const args of [null, " recover delivery-1", " abandon delivery-1", " action finish\nfinal answer", " list", " status"]) {
      expect(view({ node: { commandId: `command-${String(args)}`, name: "crystra", args, outcome: null } })).toBeNull();
    }
  });
  ...
  it.each([
    ...
    const definition = definitions.find((candidate) => candidate.kind === "crystra-interaction");
    expect(definition, "Crystra chat conversation definition").toBeDefined();
    const run = { type: "command/run", seq: 10, time: 20, data: {
      commandId: "presentation-command-1", name: "crystra", source: { kind: "plugin", plugin: "workflow-execution" },
    } };
    const done = { type: "command/done", seq: 11, time: 21, data: { commandId: "presentation-command-1", kind: "success", text: JSON.stringify({
      schemaVersion: "crystra.presentation@1.0.0", correlation: "presentation-1", kind, data,
    }) } };
    ...
    const initial = definition.start({ key: "crystra-interaction:presentation-command-1" }, startMatch);
    ...
    const node = definition.buildViewNode({
      key: "crystra-interaction:presentation-command-1", id: "presentation-command-1", state,
      start: startMatch, matches: [], current: new Map(),
    });
    expect(node).toMatchObject({ kind: "crystra-interaction", target: "chat", anchorSeq: 10, visibility: "visible" });
    const view = components.get("conversation.chat.node");
    expect(view, "Crystra chat node renderer").toBeTypeOf("function");
    const rendered = view!({ node });
    expect(rendered.props["data-crystra-surface"]).toBe("chat");
    expect(rendered.props["data-crystra-chat-role"]).toBe("assistant");
    expect(rendered.props["data-crystra-kind"]).toBe(kind);
    expect(rendered.props["data-crystra-state"]).toBe(typeof fields.state === "string" ? fields.state : undefined);
    expect(rendered.props["data-crystra-failure-stage"]).toBe(typeof fields.diagnostic?.stage === "string" ? fields.diagnostic.stage : undefined);
    expect(rendered.props["data-crystra-cause-code"]).toBe(typeof fields.diagnostic?.causeCode === "string" ? fields.diagnostic.causeCode : undefined);
  ...
  it("defensively renders only visible text from a native Action block array", async () => {
    ...
    const done = { type: "command/done", seq: 11, time: 21, data: { commandId: "presentation-command-1", kind: "success", text: JSON.stringify({
      schemaVersion: "crystra.presentation@1.0.0", correlation: "presentation-1", kind: "action-output", data: { content: [
    ...
    const actions = elementsOf(rendered).find((child: any) => child?.props?.["data-crystra-answer-actions"] === "true");
    ...
    expect(copy.props.className).toBe("crystra-answer-action");
...
describe("DSH official Crystra sidebar projection", () => {
  it("queries list/status from tabs and projects only control-plane results", async () => {
    const outcome = { text: JSON.stringify({
      schemaVersion: "crystra.presentation@1.0.0", correlation: "presentation-1", kind: "delivery-list", data: { items: [] },
    }) };
    const actionOutcome = { text: JSON.stringify({
      schemaVersion: "crystra.presentation@1.0.0", correlation: "presentation-action", kind: "action-output", data: { content: "chat only" },
    }) };
    const { components, commands } = await loadClient({
      composerPhase: "blank",
      chat: { order: ["command-1", "command-2"], nodes: new Map([["command-1", {
        kind: "command", data: { commandId: "command-1", name: "crystra", outcome },
      }], ["command-2", { kind: "command", data: { commandId: "command-2", name: "crystra", outcome: actionOutcome } }]]) },
    });
    ...
    expect(rendered.props).toMatchObject({
      ...
      "data-crystra-version": "crystra.presentation@1.0.0",
      "data-crystra-kind": "delivery-list",
    ...
    expect(commands).toEqual(["/crystra list", "/crystra status"]);
  });

  it("renders the same bounded unresolved diagnostic in the sidebar status and semantic tags", async () => {
    const outcome = { text: JSON.stringify({
      schemaVersion: "crystra.presentation@1.0.0", correlation: "presentation-unresolved", kind: "delivery-status", data: {
    ...
    expect(updated.props).toMatchObject({
      "data-crystra-state": "RESULT_UNRESOLVED",
      "data-crystra-failure-stage": "HOST_START",
      "data-crystra-cause-code": "GIT_STATE_MISMATCH",
    });
  });
});
