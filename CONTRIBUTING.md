...
- **Release authority** — this repository publishes only `crystra-execution`. DSH bundles are independently versioned and published from `firestige/crystra-dsh`; the retained legacy source is compatibility-only.
- **Gates before push** — `pnpm typecheck && pnpm build && pnpm test:full && pnpm check:generated && pnpm changelog:check`. DSH product qualification runs in `firestige/crystra-dsh`.

## Release process

See the [release process guide](https://github.com/firestige/crystra/blob/main/docs/guides/execution-release-process.md): local qualification → RC prerelease → remote verification → promote stable. The GitHub Release is the byte source of truth; ordinary consumers install its verified tarball.
